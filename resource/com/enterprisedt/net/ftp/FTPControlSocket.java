 ...
 *        Revision 1.67  2016/04/22 12:28:16  bruceb
 *        Move IPv6 constants
 *
 *        Revision 1.66  2016/02/18 00:40:13  bruceb
 *        fix equals bug
 *
 *        Revision 1.65  2016/01/30 02:40:37  hans
 *        Only show 'Substituting server supplied IP' if something has actually been substituted.
 *
 *        Revision 1.64  2015/01/14 10:02:08  bruceb
 *        include reply code in exception
 *
 *        Revision 1.63  2014/02/17 00:33:25  bruceb
 *        in PASV mode, send command on control socket THEN attempt to connect to server's listening socket, and then read reply (some servers require this)
 *
 *        Revision 1.62  2013/09/11 00:36:10  bruceb
 *        made reading reply more robust (skip nonsense)
 *
 ...
 */
 public class FTPControlSocket {
      ...
      *  Used to detect IPv6
      */
     private final static String INET6_ADDRESS_CLASS = "Inet6Address";

     /**
      * Start of PASV reply in IPv6
      */
     protected final static String EPSV_START = "(|||";

     /**
      * End of PASV reply in IPv6
      */
     protected final static String EPSV_END = "|)";
	...
	protected FTPDataSocket newPassiveDataSocket(String remoteHost, int port) 
	    ...
	    return new FTPPassiveDataSocket(remoteHost, port, controlSock.getSoTimeout());
	}
     ...
     * @throws IOException, FTPException 
     ...
     /**
      * Checks if we have a valid first line of reply - must always
      * start with 3 digit reply code
      * 
      * @param line
      * @return true if valid, false otherwise
      */
     private boolean isValidFirstLineOfReply(String line) {
     ...
     FTPReply readReply()
         ...
         while (!isValidFirstLineOfReply(line)) {
             log.debug("Skipping invalid reply line: '" + line + "'");
             line = readLine();
         }
         ...
         if (line.length() > 3 && line.charAt(3) == '-') {
             ...
             while (!complete) {
                 ...
                 if (line.length() > 3 &&
                         isNumeric(line.substring(0, 3)) &&
                         line.charAt(3) == ' ') {
                     
                     // piece of crap server TCP/IP for VSE Internal FTPDAEMN 01.05 doesn't match reply codes!!
                     // so we set the reply code to the last valid code in the multiline reply rather than 
     ...
     private boolean validateReplyCode(FTPReply reply, String expectedReplyCode) 
         ...
         if ("421".equals(replyCode)) {
             throw new FTPConnectionClosedException(reply.getReplyText(), replyCode);
         }
     ...
     protected boolean isIPV6(InetAddress host) {
         ...
         String hostClass = host.getClass().getName();
         log.debug("Checking if IPV6:" + hostClass + " (" + host.getHostAddress() + ")");
         if (hostClass.indexOf(INET6_ADDRESS_CLASS) < 0)
             return false;
         else
             return true;
     }
