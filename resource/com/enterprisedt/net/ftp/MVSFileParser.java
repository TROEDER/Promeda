 ...
 *    Revision 1.4  2015/01/30 22:45:12  bruceb
 *    tweaks to get date format
 *
 *    Revision 1.3  2015/01/06 11:30:55  bruceb
 *    tweak to cope with new format
 *
 *    Revision 1.2  2013/07/31 03:50:11  bruceb
 *    minor tweaks for yet another different line format in listing
 *
...
public class MVSFileParser extends FTPFileParser {
  ...
  */
  ...
  protected FTPFile parsePDSLine(String[] aLine, String raw) throws ParseException {
    ...
    if (dateTimeFormat == null && dateFormatString != null) {
  ...
  protected FTPFile parseFolder(String[] aLine, String raw) {
    ...
    if (aLine.length != 10) { // It's been moved.
        log.debug("Skipping invalid line: " + raw); //$NON-NLS-1$
        return null;
     }
  ...
  protected boolean isValidDirectoryFormat(String[] listing) {
    ...
    for (int i=1; i<listing.length; i++) {
      ...
      if ( (aLine.length == 2) && (aLine[0].equals(LINE_TYPE_MIGRATED))) {
        ...
        log.debug("Invalid Directory Line: " + listing[i]); //$NON-NLS-1$
        continue;
      }
