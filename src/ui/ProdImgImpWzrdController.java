	...
	public void initStores() {
		...
		for (File file : files) {
			if (!file.isDirectory()) {
				...
				for (String size : propStore.get("img.sizes").split(",")) {
					int imgSizeInt = Integer.parseInt(size);

				}
				stores.add(new StoreDataModel(propStore.get("url"), propStore.get("ftp.host"),
						Integer.parseInt(propStore.get("ftp.port")), propStore.get("ftp.user"), propStore.get("ftp.pswd"), imgSizes));
			}
	...
	public void process() {
		...
		try {
			for (StoreDataModel store : selectedStores) {
				...
				ftp = new FtpClientModel(store.getStoreFtpServer(), store.getStoreFtpPort(),
						store.getStoreFtpUser(), store.getStoreFtpPass());

				for (File psdFile : psdFileList) {

					// String[] fileNameParts = psdFile.getName().split("\\.(?=[^\\.]+$)"); // Not
					// used

					// COPY PSD FILE TO ORIGINALS FOLDER
					copyFile(psdFile, new File("media/originals/" + FilenameUtils.getBaseName(psdFile.getName())
					...
					for (String imgSize : store.getStoreImageSizes()) {
						...
						File directory = new File(
								"media/live/" + imgSize + "px/" + FilenameUtils.getBaseName(psdFile.getName()));
						if (!directory.exists()) {
							directory.mkdirs();
						}
...
//							ftp.upload(imgFile.getPath(), imgSize + "px/" + imgFile.getName());
							File remoteFile = new File(imgSize + "px/" + imgFile.getName());
							ftp.sftpConnect();
